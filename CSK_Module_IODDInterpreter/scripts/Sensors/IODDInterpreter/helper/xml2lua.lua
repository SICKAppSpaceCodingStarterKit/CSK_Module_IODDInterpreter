...
--@see XmlParser
function xmlInit.parser(handler)    
    if handler == xmlInit then
        error("You must call xmlInit.parse(handler) instead of xmlInit:parse(handler)")
    end
...
function xmlInit.loadFile(xmlFilePath)
...
local function parseTableKeyToXml(xmltb, tagName, fieldValue, level)
    ...
    if type(fieldValue) == "table" then
        ...
        strValue = #fieldValue == 1 and spaces..tostring(fieldValue[1]) or xmlInit.toXml(fieldValue, tagName, level+1)
        strValue = '\n'..strValue..'\n'..spaces
    else
...
function xmlInit.toXml(tb, tableName, level)
  level = level or 1
...
local function removeAttr(tableWithAttr)
  for key, value in pairs(tableWithAttr) do
    if type(value) == 'table' then
      if value._attr then
        for i,v in pairs(value._attr) do
          tableWithAttr[key][i] = v
        end
        tableWithAttr[key]._attr = nil
      end
      removeAttr(value)
    end
  end
...
local function convert(xmlString)
  local xml2intiator = xmlInit
  local xmlHandler = tree:new()
  local parser = xml2intiator.parser(xmlHandler)
  parser:parse(xmlString)
  local newLuaTable = xmlHandler.root
  removeAttr(newLuaTable)
  return newLuaTable
end
xml2lua.convert = convert
